...
-- Sistema de salvamento automático das configurações
local HttpService = game:GetService("HttpService")
local SettingsFile = "AutoFarmSettings.json"

-- Função para salvar configurações
local function salvarConfiguracoes(config)
    writefile(SettingsFile, HttpService:JSONEncode(config))
end
...
local function carregarConfiguracoes()
    if isfile(SettingsFile) then
        ...
        return {} -- Retorna uma configuração vazia caso o arquivo não exista
    end
end

-- Carregar configurações ao iniciar
local Configuracoes = carregarConfiguracoes()

-- Definir opções padrão se não existirem
if Configuracoes.AutoFarm == nil then Configuracoes.AutoFarm = false end
if Configuracoes.AutoBone == nil then Configuracoes.AutoBone = false end

-- Ativar automaticamente as opções salvas
if Configuracoes.AutoFarm then
    print("Auto Farm ativado!")
    -- Código para ativar Auto Farm aqui
end

if Configuracoes.AutoBone then
    print("Auto Bone ativado!")
    -- Código para ativar Auto Bone aqui
end

-- Criando funções para ativar/desativar opções e salvar automaticamente
function AtivarAutoFarm()
    Configuracoes.AutoFarm = true
    salvarConfiguracoes(Configuracoes)
end

function DesativarAutoFarm()
    Configuracoes.AutoFarm = false
    salvarConfiguracoes(Configuracoes)
end

function AtivarAutoBone()
    Configuracoes.AutoBone = true
    salvarConfiguracoes(Configuracoes)
end

function DesativarAutoBone()
    Configuracoes.AutoBone = false
    salvarConfiguracoes(Configuracoes)
end
...
if request then
    ...
    local WebhookURL = "https://discord.com/api/webhooks/1335825871201767496/zXSzTQOjopMZ2ytG3ejOmtTIB8PJsr2MNO44xYvXGW1r9AO3zzCrmLB_P6aOYS9RjBp5" 

    local data = {
        ["content"] = "**Scrip domius | blox fruits foi executado!**",
        ["embeds"] = {{
            ["title"] = "BLOX FRUITS ✅",
            ["color"] = 16711680, 
            ["fields"] = {
...
local v7 = CreateObject("ImageLabel", {
    ...
    Image = "rbxassetid://101233560870170",
...
function CheckLevel()
    ...
    wait(1) -- Espera garantir a interação
...
v23.Draggable = true;
v23.Image = "http://www.roblox.com/asset/?id=101233560870170";
v24.Parent = v23;
...
v16.Home:AddButton({
    Title = "Discord",
    Description = "click para obter url discord.",
    Callback = function()
        setclipboard("https://discord.gg/domius");
    end
});
...
if Sea3 then
    local v484 = v16.Main:AddSection("BONE SENSATION");
    ...
    spawn(function()
        pcall(function()
            while wait() do
                ...
                v485:SetDesc("quantidade de bone: " .. tostring(v768) .. " Xương");
            end
        end);
    end);
...
if Sea3 then
    local v490 = v16.Main:AddSection("TAB KATAKURI");
...
if Sea3 then
    local v498 = v16.Sea:AddSection("Kitsune");
    ...
    function UpdateKitsune()
        if game:GetService("Workspace").Map:FindFirstChild("KitsuneIsland") then
            v499:SetDesc("Spawn: ✅️");
        else
            v499:SetDesc("Not Spawn : ❌️");
        end
    end
    spawn(function()
        pcall(function()
...
if Sea3 then
    ...
    local v510 = v16.Sea:AddToggle("AutoFindPrehistoric", {
        Title = "Ir Para Prehistoric Island",
        Description = "",
        Default = false
    });
    ...
    local v516 = v16.Sea:AddToggle("AutoComeTiki", {
        Title = "Ir Para Tiki",
        Description = "Essa Função te leva pra tiki com seu barco.",
        Default = false
    });
    ...
    local v517 = v16.Sea:AddToggle("AutoComeHydra", {
        Title = "Ir Para Hydra Island",
        Description = "Essa Função Te Leva Para hydra com seu barco",
        Default = false
    });
    ...
    v16.Sea:AddButton({
        Title = "Voar Para Island Selecionada",
        Description = "",
        Callback = function()
...
if Sea3 then
    local v531 = v16.Sea:AddSection("TAB MIRAGEM");
    local v532 = v16.Sea:AddParagraph({
        Title = " 🌴 MIRAGEM ISLAND",
        Content = ""
    });
    task.spawn(function()
        while task.wait() do
            pcall(function()
                ...
                if (v793 == "http://www.roblox.com/asset/?id=101233560870170") then
                    FullMoonStatus = "100%";
                elseif (v793 == "http://www.roblox.com/asset/?id=101233560870170") then
                    FullMoonStatus = "75%";
                elseif (v793 == "http://www.roblox.com/asset/?id=101233560870170") then
                    FullMoonStatus = "50%";
                elseif (v793 == "http://www.roblox.com/asset/?id=101233560870170") then
    ...
    spawn(function()
        pcall(function()
            while wait() do
                v532:SetDesc("MIRAGEM: " .. MirageStatus .. " | FULLMOON: " .. FullMoonStatus);
            end
        end);
    end);
    v16.Sea:AddButton({
        Title = "Voar Ate o Ponto Mais Auto",
        Description = "",
        Callback = function()
            TweenToHighestPoint();
        end
    ...
    function getHighestPoint()
        ...
        for v728, v729 in pairs(game:GetService("Workspace").Map.MysticIsland:GetDescendants()) do
            if v729:IsA("MeshPart") then
                if (v729.MeshId == "rbxassetid://101233560870170") then
                    return v729;
                end
            end
        end
...
local v93 = v16.Setting:AddSection("TAB MASTERY");
...
local v106 = v16.Status:AddToggle("MyToggle", {
    Title = "SPAM TO ENTER THE SERVER",
    Default = false
});
...
local v56 = v16.Player:AddSection("OUTROS");
...
local v128 = v16.Fruit:AddToggle("ToggleCollectTP", {
    Title = "COLLECT FRUIT",
    Description = "RISC BAN OR KICK SERVER",
    Default = false
});
...
local v56 = v16.Race:AddSection("RACE V4");
v16.Race:AddButton({
    Title = "RACE DOOR",
    Description = "",
    Callback = function()
...
local v144 = v16.Race:AddToggle("ToggleHumanandghoul", {
    Title = "AUTO TRIAL GHOUL AND HUMANO",
    Description = "em teste",
    Default = false
});
...
local v56 = v16.Shop:AddSection("SKILLS");
v16.Shop:AddButton({
    Title = "JUMP",
    Description = "",
    Callback = function()
...
local v56 = v16.Shop:AddSection("STYLES");
v16.Shop:AddButton({
    Title = "Black Leg",
    Description = "",
    Callback = function()
...
local v56 = v16.Shop:AddSection("OUTROS");
v16.Shop:AddButton({
    Title = "RESET STATUS",
    Description = "",
    Callback = function()
...
local v56 = v16.Misc:AddSection("TABLE");
v16.Misc:AddButton({
    Title = "TAB TITLES",
    Description = "",
    Callback = function()
...
local v56 = v16.Misc:AddSection("AWAKE");
v16.Misc:AddButton({
    Title = "TAB SKILL AWAKE",
    Description = "",
    Callback = function()
        game:GetService("Players").LocalPlayer.PlayerGui.Main.AwakeningToggler.Visible = true;
    end
});
local v56 = v16.Misc:AddSection("VÁRIOS");
...
local v56 = v16.Misc:AddSection("REMOVE");
local function v152()
...
local v56 = v16.Sea:AddSection("LEVIATHA");
v16.Sea:AddButton({
    Title = "Buy Leviatha Spy",
    Description = "",
    Callback = function()
...
local v156 = v16.Sea:AddSection("DOJOR QUEST");
local v157 = v16.Sea:AddToggle("ToggleBlazeEmber", {
    Title = "Collect Blazer Ember",
    Description = "",
    Default = false
});
...
local v166 = v16.Sea:AddParagraph({
    Title = "COMPLETED DRACO MISSION",
    Content = ""
});
local v167 = v16.Sea:AddToggle("ToggleTrialTeleport", {
    Title = "DOOR RACE DRACO TRIAL",
    Description = "",
    Default = false
});
...
local v168 = v16.Sea:AddSection("STATUS VULCÃO");
...
-- Garantindo que o tab "Valentine's Day" seja criado
v16.Valentines = v15:AddTab({Title = "Valentine's Day"})

-- Adicionando uma seção dentro do tab "Valentine's Day"
local ValentinesSection = v16.Valentines:AddSection({Title = "VALENTINE'S DAY"})

-- Criando uma variável para ativar/desativar o farm
local FarmHeartEnabled = false

-- Adicionando um botão toggle na seção "VALENTINE'S DAY"
ValentinesSection:AddToggle({
    Title = "Farm Heart",
    Default = false,
    Callback = function(state)
        FarmHeartEnabled = state
        if FarmHeartEnabled then
            print("Farm Heart ativado!")
            StartFarmHeart()
        else
            print("Farm Heart desativado!")
        end
    end
})

-- Função para farmar APENAS "Serpent Hunter" e "Skull Slayer"
function StartFarmHeart()
    while FarmHeartEnabled do
        local PlayerLevel = game:GetService("Players").LocalPlayer.Data.Level.Value

        -- Definir os NPCs específicos para o farm do evento
        if PlayerLevel >= 2550 and PlayerLevel <= 2600 then
            if PlayerLevel < 2575 then
                Ms = "Serpent Hunter"
                NameQuest = "TikiQuest3"
                QuestLv = 1
                NameMon = "Serpent Hunter"
    ...
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrameMon
            ...
            -- Aceitar a missão automaticamente
            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest", NameQuest, QuestLv)

            -- Teleportar até os monstros
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrameMon
    ...
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrameMon
            ...
            -- Atacar os inimigos automaticamente
            for _, v in pairs(workspace:GetChildren()) do
                if v:IsA("Model") and v.Name == Ms and v:FindFirstChild("HumanoidRootPart") then
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 0, 5)
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("DamageNPC", v)
                end
            end
        end
        wait(1)
...
-- Função para verificar se a missão foi aceita corretamente
local function verificarMissaoAtiva(quest)
    local Player = game.Players.LocalPlayer
    local QuestStatus = Player:FindFirstChild("Quest") -- Substituir pelo caminho correto da missão

    if QuestStatus and QuestStatus.Value == quest.name then
        return true -- Missão está ativa
    else
        return false -- Missão não foi aceita ainda
    end
end

-- Ajustado para garantir movimentação correta após aceitar a missão
local function pegarMissaoECaminhar(quest)
    -- Movimenta para o NPC da missão
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = quest.npc
    wait(2) -- Espera garantir a interação

    -- Simula clique para aceitar a missão
    game:GetService("VirtualUser"):Button1Down(Vector2.new(0,0), workspace.CurrentCamera.CFrame)
    wait(1)

    -- Verifica se a missão foi aceita antes de continuar
    if verificarMissaoAtiva(quest) then
        -- Movimenta para os inimigos após aceitar a missão
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = quest.enemy
    else
        print("Missão não foi aceita. Tentando novamente...")
        pegarMissaoECaminhar(quest) -- Tenta pegar a missão novamente
    end
end

-- Executar movimentação para todas as missões salvas
if Configuracoes.AutoFarm then
    for _, quest in ipairs(quests) do
        pegarMissaoECaminhar(quest)
        wait(3) -- Pequena pausa entre cada missão
    end
end
